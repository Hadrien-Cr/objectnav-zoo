...
from obs_iin_preprocessor import ObsIINPreprocessor
...
class ModIINAgentModule(nn.Module):
    """
...
class ModIINAgent(Agent):
    ...
    def __init__(self, config: DictConfig, device_id: int = 0) -> None:
        ...
        self.obs_preprocessor = ObsIINPreprocessor(config, self.device)

        self.max_steps = config.habitat.environment.max_episode_steps
        self.num_environments = 1

        self._module = ModIINAgentModule(config).to(self.device)
